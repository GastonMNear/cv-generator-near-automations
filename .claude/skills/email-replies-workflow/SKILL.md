...
**Symptoms:** Table name doesn't match any known tables or API search fails

**Solutions:**
1. Check spelling of table name
2. Use exact table name from known-tables.md
3. Try providing table ID directly (format: t_[alphanumeric])
4. Run manual API call to list all tables:
   ```
   GET /workspaces/447061/tables
   ```

### Issue: "Lead email not found"

**Symptoms:** Email search completes but no record found

**Solutions:**
1. Verify email spelling and format
2. Check lead exists in specified table (view table in Clay web app)
3. Try searching by email domain only
4. Confirm email field is correct (may be in different field)
5. Check if lead was recently added (might not be indexed yet)

### Issue: "LinkedIn URL field is empty"

**Symptoms:** Lead found but LinkedIn URL field has no value

**Solutions:**
1. Check Clay table in web app - confirm URL is populated
2. Verify correct LinkedIn URL field is being used (not prospect LinkedIn)
3. If job URL is in different field, specify exact field name
4. Cannot proceed without job URL - lead must have job posting URL

### Issue: "Invalid LinkedIn URL format"

**Symptoms:** URL extracted but doesn't match expected pattern

**Solutions:**
1. Verify URL is job posting, not prospect profile
2. Check for typos in URL field
3. Ensure URL follows format: linkedin.com/jobs/view/[job_id]
4. Try opening URL in browser to verify it's accessible

### Issue: "Google Doc creation failed"

**Symptoms:** CV generated but Google Doc not created
...
2. Verify GOOGLE_DRIVE_FOLDER_ID exists and is accessible
3. Run google_auth_setup.py to refresh OAuth tokens
4. Check Google Drive API quota/limits
5. Do NOT accept CV markdown as fallback — report the error and ask user to fix credentials

### Issue: "Clay authentication fails repeatedly"

**Symptoms:** Login returns 401 or 429 errors

**Solutions:**
1. Verify Clay credentials in .env
2. Check if Clay password contains special characters (properly escape in .env)
3. If rate limited (429): wait 60 seconds before retrying
4. Try logging in via web browser to confirm credentials work
5. If persistent: contact Clay support

### Issue: "Field resolution ambiguity"

**Symptoms:** Multiple fields match pattern, unclear which to use

**Solutions:**
1. Review field list returned by error message
2. Specify exact field name or ID
3. Check field descriptions in Clay web app
4. Update field-name-patterns.md with clarified pattern
5. Consider adding field mapping to known-tables.md
